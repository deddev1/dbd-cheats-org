 ...
 * Composited on top of IGN Dead by Daylight gameplay screenshots.
 */
...
export function modMenuOverlay(w, h) {
	...
	return `
		...
		<text x="${x + 14}" y="${y + 28}" fill="${PURPLE}" font-family="Segoe UI,Arial,sans-serif" font-size="15" font-weight="800">DBD CHEATS</text>
...
export function hacksBadge(w, h) {
	return `
		...
		<text x="${w - 160}" y="${h - 18}" fill="${PURPLE}" font-family="Segoe UI,Arial,sans-serif" font-size="12" font-weight="700">DBD CHEATS</text>
	`;
}
...
export function buildOverlaySvg(width, height, preset) {
	...
	switch (preset) {
		...
		case 'loot':
			...
			body += espBox(width * 0.68, height * 0.52, width * 0.06, height * 0.08, 'trial timer · 103m', '#fbbf24');
